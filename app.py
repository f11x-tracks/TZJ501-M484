...
def make_radius_thickness_by_condition_plots(y_range=None, show_trend_legend=True, filtered_df=None):
    ...
    unique_conditions = sorted(thickness_data['Condition'].unique(), 
                              key=lambda x: int(x.split('_')[0]) if x and '_' in x else 0)
...
def make_statistical_summary_table(filtered_df=None):
    ...
    # Get unique wafer IDs and sort by their condition
    wafer_condition_map = working_df.groupby('WaferID')['Condition'].first().to_dict()
    unique_wafers = sorted(working_df['WaferID'].unique(), 
                          key=lambda x: (int(wafer_condition_map.get(x, 'NA').split('_')[0]) 
                                        if wafer_condition_map.get(x, 'NA') != 'NA' and '_' in wafer_condition_map.get(x, 'NA') 
                                        else 999, x))
...
def make_statistical_summary_by_condition_table(filtered_df=None):
    ...
    for condition in unique_conditions:
        ...
        # Parse combined condition
        base_condition, condition2 = parse_combined_condition(condition)
...
def get_filtered_dataframe(outlier_method='none', outlier_threshold=1.5, condition_filter=''):
    """Get filtered dataframe based on outlier detection settings and condition filter"""
    working_df = df.copy()
    
    # Apply condition filter first if provided
    if condition_filter and condition_filter.strip():
        # Case-insensitive partial match
        condition_mask = working_df['Condition'].str.contains(condition_filter.strip(), case=False, na=False)
        working_df = working_df[condition_mask]
    
    # Then apply outlier filtering
    if outlier_method == 'none' or working_df.empty:
        return working_df
...
def update_thickness_boxplot(yscale_percent, outlier_method, outlier_threshold, condition_filter):
...
def update_radius_thickness_plots(yscale_percent, show_trend_legend, outlier_method, outlier_threshold, condition_filter):
    filtered_df = get_filtered_dataframe(outlier_method, outlier_threshold, condition_filter)
...
def update_radius_thickness_condition_plots(yscale_percent, show_trend_legend, outlier_method, outlier_threshold, condition_filter):
    filtered_df = get_filtered_dataframe(outlier_method, outlier_threshold, condition_filter)
...
def update_wafer_summary_table(outlier_method, outlier_threshold, condition_filter):
    filtered_df = get_filtered_dataframe(outlier_method, outlier_threshold, condition_filter)
...
def update_condition_summary_table(outlier_method, outlier_threshold, condition_filter):
    filtered_df = get_filtered_dataframe(outlier_method, outlier_threshold, condition_filter)
...
def update_condition_average_thickness_plot(outlier_method, outlier_threshold, condition_filter):
    filtered_df = get_filtered_dataframe(outlier_method, outlier_threshold, condition_filter)
...
def update_condition_std_dev_plot(outlier_method, outlier_threshold, condition_filter):
    filtered_df = get_filtered_dataframe(outlier_method, outlier_threshold, condition_filter)
...
def update_outlier_status(outlier_method, outlier_threshold, condition_filter):
    if outlier_method == 'none' and not condition_filter:
        return html.Div()
    ...
    # Add condition filter status
    if condition_filter and condition_filter.strip():
        condition_filtered_df = df[df['Condition'].str.contains(condition_filter.strip(), case=False, na=False)]
        condition_filtered_count = len(condition_filtered_df[condition_filtered_df['Label'] == 'Layer 1 Thickness'])
        status_messages.append(
            html.P(f"� Condition Filter Active: '{condition_filter}' - Showing {condition_filtered_count} out of {original_count} measurements", 
                   style={'color': 'blue', 'fontWeight': 'bold', 'margin': '5px 0'})
        )
    
    # Add outlier filter status
    if outlier_method != 'none':
        ...
        if removed_count > 0:
            status_messages.append(
                html.P(f"📊 Total filtered: {removed_count} measurements removed ({removed_count/original_count*100:.1f}%)", 
...
def make_condition_average_thickness_plot(filtered_df=None):
    ...
    # Sort by condition name numerically for consistent display
    # Handle both condition number and wafer number for proper sorting
    def sort_condition(condition_series):
        def parse_condition(cond):
            try:
                # Handle parentheses format: "1_Wfr6 (13-1)" -> extract "1_Wfr6"
                base_condition = cond.split(' (')[0] if ' (' in cond else cond
                parts = base_condition.split('_')
                if len(parts) == 2:
                    cond_num = int(parts[0])
                    wfr_part = parts[1]  # e.g., 'Wfr6'
                    if wfr_part.startswith('Wfr'):
                        wfr_num = int(wfr_part[3:])  # Extract number after 'Wfr'
                        return (cond_num, wfr_num)
                    else:
                        return (cond_num, 0)  # Fallback
                else:
                    return (0, 0)  # Fallback for unexpected format
            except (ValueError, IndexError):
                return (0, 0)  # Fallback for parsing errors
        
        return condition_series.map(parse_condition)
    
    condition_stats = condition_stats.sort_values('Condition', key=sort_condition)
    ...
    fig.update_layout(
        ...
        margin=dict(l=50, r=50, t=50, b=120),  # Increased bottom margin for rotated labels
        showlegend=False
    )
    ...
    if len(condition_stats) > 5:
        fig.update_layout(
            ...
            xaxis=dict(
                tickmode='array',
                tickvals=list(range(len(condition_stats))),
                ticktext=condition_stats['Condition'].tolist()
            )
        )
    else:
        # For fewer conditions, ensure all text is visible
        fig.update_layout(
            xaxis=dict(
                tickmode='array', 
                tickvals=list(range(len(condition_stats))),
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([
            html.Label("Condition Filter:", style={'fontWeight': 'bold', 'marginRight': '10px'}),
            dcc.Input(
                id='condition-filter-input',
                type='text',
                placeholder='Enter condition to filter (e.g., rpm, no-relax)',
                value='',
